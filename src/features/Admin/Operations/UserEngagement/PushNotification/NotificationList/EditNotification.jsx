...
import { updateDraftAsync, uploadImageAsync } from "../../userEngagementSlice";
import Remove from "../../../../../../assets/icons/Remove.svg";
import {navigationParentCategory, navigationDoctorCategory} from '../../findValue';
...
export default function EditNotification(data) {
  ...
  const [uType, setuType] = React.useState(
    rowData.usersType ? rowData.usersType : ""
  );
  const [pType, setpType] = React.useState(
    rowData.parentSubscriptionPlan ? rowData.parentSubscriptionPlan : null
  );
  const [psType, setpsType] = React.useState(
    rowData.parentSubCategory ? rowData.parentSubCategory : null
  );

  const [navigationSetup, setNavigationSetup] = React.useState(
    rowData.navigationSetup ? rowData.navigationSetup : null
  );
  ...
  const [subject, setSubject] = useState(
    rowData.subject ? rowData.subject : null
  );
  const [message, setMessage] = useState(
    rowData.message ? rowData.message : null
  );
  const [image, setImage] = useState(rowData.image ? rowData.image : null);
  const [countryId, setCountryId] = useState(
    rowData.countryId ? rowData.countryId : null
  );
  const [countryName, setCountryName] = useState(
    rowData.country ? rowData.country : null
  );
  const [ageFromMonth, setAgeFromMonth] = useState(
    rowData.ageFromMonth || rowData?.ageFromMonth == 0
      ? rowData.ageFromMonth
      : null
  );
  const [ageToMonth, setAgeToMonth] = useState(
    rowData.ageToMonth ? rowData.ageToMonth : null
  );
    ...
    <React.Fragment>
        ...
        <form noValidate autoComplete="off">
          <Box p={3}>
            <Grid container spacing={3}>
              ...
              {psType !== null &&
              psType === "With premium expiration within" ? (
                <Grid item xs={12} md={6}>
                  <TextField
                    id="days"
                    label="Days"
                ...
                psType === "With Premium already Expired" ? (
                <Grid item xs={12} md={6}>
                  <TextField
                    id="days"
                    label="Days"
              ...
              <Grid item xs={12} md={6}>
                {((imgWidth && imgWidth !== 400) ||
                  ...
                  <FormHelperText>
                    Acceptable Width : 400px height: 200px
                  </FormHelperText>
                )}
                {((imgWidth && imgWidth !== 400) ||
                  (imgHeight && imgHeight !== 200)) && (
              ...
              {image && (
                <TextField
                  disabled={true}
                  fullWidth
                  value={`Selected image: ${image ? image : "Not Selected"}`}
                ></TextField>
              )}
                
              {(uType === "PARENTS" || uType === "DOCTORS") && (
                <Grid item xs={12} md={6}>
                  <Autocomplete
                    id="navigationCategory"
                    options={uType === "PARENTS" ? navigationParentCategory : navigationDoctorCategory}
                    getOptionLabel={(option) => option.title}
                    fullWidth
                    size="small"
                    renderInput={(params) => (
                      <TextField
                        {...params}
                        label="Select navigation point"
                        variant="outlined"
                      />
                    )}
                    onChange={(event, value) => setNavigationSetup(value.title)}
                    defaultValue={{ title: navigationSetup }}
                  />
                </Grid>
              )}
      ...
      <DialogActions>
        <Button
          ...
          color="secondary"
          endIcon={<SendIcon />}
          disabled={!subject || !message || isLoadin === "loading"}
          onClick={() => {
            handleAllSend({
              id: rowData.id,
              usersType: uType,
